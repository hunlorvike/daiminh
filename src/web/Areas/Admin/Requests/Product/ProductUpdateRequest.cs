...
public class ProductUpdateRequestValidator : AbstractValidator<ProductUpdateRequest>
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ProductUpdateRequestValidator"/> class.
    /// </summary>
    public ProductUpdateRequestValidator(ApplicationDbContext context)
    {
        _dbContext = context;

        RuleFor(request => request.Id)
            .GreaterThan(0).WithMessage("ID sản phẩm phải là một số nguyên dương.")
            .MustAsync(BeExistingProduct).WithMessage("Sản phẩm không tồn tại.");
    ...
    private async Task<bool> BeExistingProduct(int id, CancellationToken cancellationToken)
    {
        return await _dbContext.Products
            .AnyAsync(p => p.Id == id && p.DeletedAt == null, cancellationToken);
    }

    private async Task<bool> BeUniqueSlug(ProductUpdateRequest request, string slug, CancellationToken cancellationToken)
    {
        return !await _dbContext.Products
            .AnyAsync(p => p.Slug == slug && p.Id != request.Id && p.DeletedAt == null, cancellationToken);
    }

    private async Task<bool> BeUniqueSku(ProductUpdateRequest request, string sku, CancellationToken cancellationToken)
    {
        return !await _dbContext.Products
            .AnyAsync(p => p.Sku == sku && p.Id != request.Id && p.DeletedAt == null, cancellationToken);
    }
